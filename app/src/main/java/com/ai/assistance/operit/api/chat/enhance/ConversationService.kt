...
class ConversationService(
    ...
    suspend fun prepareConversationHistory(
        ...
        // Compresie proactivă: previne umflarea contextului și "uitarea" începutului.
        // Aplicată DUPĂ hooks pentru ca hook-urile să poată modifica istoricul,
        // dar ÎNAINTE de trimiterea finală către model.
        val historyAfterCompression = compressOversizedHistory(afterContext.preparedHistory)
        // Rezumat prioritar: mută orice turn SUMMARY imediat după SYSTEM,
        // astfel încât memoria comprimată să fie mereu vizibilă pentru model
        // (nu îngropată în mijlocul istoricului).
        val systemTurns = historyAfterCompression.filter { it.kind == PromptTurnKind.SYSTEM }
        val summaryTurns = historyAfterCompression.filter { it.kind == PromptTurnKind.SUMMARY }
        val otherTurns = historyAfterCompression.filter {
            it.kind != PromptTurnKind.SYSTEM && it.kind != PromptTurnKind.SUMMARY
        }
        val prioritizedHistory =
            if (summaryTurns.isEmpty()) {
                historyAfterCompression
            } else {
                systemTurns + summaryTurns + otherTurns
            }
        AppLogger.d(
            TAG,
            "Istoric final pregătit: ${prioritizedHistory.size} turnuri " +
                "(system=${systemTurns.size}, summary=${summaryTurns.size}, altele=${otherTurns.size})"
        )
        return prioritizedHistory
    }

    /**
     * Compresie proactivă a istoricului: când numărul de mesaje/turnuri depășește
     * pragul, păstrăm SYSTEM + primele schimburi (context important) + ultimele
     * turnuri recente, iar mijlocul este comprimat într-un rezumat local compact.
     * Scop: modelul nu "uită" începutul conversației din cauza ferestrei de context.
     */
    private fun compressOversizedHistory(
        preparedHistory: List<PromptTurn>,
        maxTurns: Int = 40,
        keepHeadTurns: Int = 6,
        keepTailTurns: Int = 24
    ): List<PromptTurn> {
        if (preparedHistory.size <= maxTurns) {
            return preparedHistory
        }
        // Separă SYSTEM (primul turn) de restul
        val systemTurns = preparedHistory.filter { it.kind == PromptTurnKind.SYSTEM }
        val rest = preparedHistory.filter { it.kind != PromptTurnKind.SYSTEM }
        if (rest.size <= keepHeadTurns + keepTailTurns) {
            return preparedHistory
        }
        val head = rest.take(keepHeadTurns)
        val tail = rest.takeLast(keepTailTurns)
        val middle = rest.subList(keepHeadTurns, rest.size - keepTailTurns)
        // Rezumat local: extrage esența din mijloc fără apel AI (gratuit, instant)
        val middleSummary = buildString {
            append("<compressed_history>\n")
            append("Următoarele schimburi anterioare au fost comprimate pentru a păstra contextul:\n")
            var userCount = 0
            var assistantCount = 0
            var toolCount = 0
            val keyUserMessages = mutableListOf<String>()
            middle.forEach { turn ->
                when (turn.kind) {
                    PromptTurnKind.USER -> {
                        userCount++
                        val snippet = turn.content.trim().lineSequence().firstOrNull().orEmpty()
                        if (snippet.length > 120) {
                            keyUserMessages.add(snippet.take(120) + "...")
                        } else if (snippet.isNotBlank()) {
                            keyUserMessages.add(snippet)
                        }
                    }
                    PromptTurnKind.ASSISTANT -> assistantCount++
                    PromptTurnKind.TOOL_CALL, PromptTurnKind.TOOL_RESULT -> toolCount++
                    else -> Unit
                }
            }
            append("- schimburi comprimate: $userCount utilizator, $assistantCount asistent, $toolCount apeluri de instrumente\n")
            if (keyUserMessages.isNotEmpty()) {
                append("- subiecte cheie din utilizator: ")
                append(keyUserMessages.take(6).joinToString(" | "))
                append("\n")
            }
            append("</compressed_history>\n")
        }
        val compressedTurn =
            PromptTurn(
                ...
                content = middleSummary.trim()
            )
        AppLogger.d(
            TAG,
            "Compresie proactivă: ${preparedHistory.size} turnuri → " +
                "${systemTurns.size + head.size + 1 + tail.size} turnuri " +
                "(comprimat mijlocul: ${middle.size} turnuri)"
        )
        return systemTurns + head + listOf(compressedTurn) + tail
    }
